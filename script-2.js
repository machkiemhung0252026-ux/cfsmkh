...
// ---------- Chặn viết CFS cho tới khi đọc & đồng ý lưu ý ----------
const RULES_ACCEPTED_KEY = 'cfsRulesAccepted';
...
function hasAcceptedRules(){
  try{ return localStorage.getItem(RULES_ACCEPTED_KEY) === '1'; }
  catch(e){ return false; }
}

function unlockTextarea(){
  contentTextarea.removeAttribute('readonly');
  contentTextarea.placeholder = "Viết điều bạn muốn nói ở đây...";
}
...
// Nếu trước đó đã đồng ý rồi thì mở khoá luôn, không cần bắt đọc lại
if (hasAcceptedRules()) {
  unlockTextarea();
}

// Người dùng bấm/chạm vào ô viết mà chưa đồng ý -> chặn lại, mở popup lưu ý
...
rulesCheckbox.addEventListener('change', () => {
  rulesAcceptBtn.disabled = !rulesCheckbox.checked;
});

rulesAcceptBtn.addEventListener('click', () => {
  if (!rulesCheckbox.checked) return;
  try{ localStorage.setItem(RULES_ACCEPTED_KEY, '1'); }catch(e){}
  unlockTextarea();
  closeRulesModal();
  contentTextarea.focus();
});

// Chạm ra ngoài card để đóng popup (không tính là đã đồng ý)
...
submitBtn.addEventListener('click', async () => {
  ...
  if (!hasAcceptedRules()) {
    openRulesModal();
    showStatus("Vui lòng đọc và đồng ý lưu ý trước khi gửi.", false);
    return;
  }
